...
class GPTDataset(MegatronDataset):
    ...
    def _query_document_sample_shuffle_indices(
        ...
        # Sample spans multiple documents
            ...
            ############################################################################
            # 跑random的时候把这个block的代码解注释，再把下面那个block的代码注释！！
            #
            # # ####this is my revised code!!!!! revise this when change to random task
            # dataset_size = len(self.dataset)  
    
            # # 修改这里，让i按照自然顺序增长，但要确保不超出数据集范围
            # start_doc_id = self.document_index[doc_index_beg]
            # for i in range(doc_index_end - doc_index_beg + 1):
            #     # Add the document id - 确保不超出范围
            #     current_doc_id = start_doc_id + i
            #     if current_doc_id >= dataset_size:
            #         current_doc_id = current_doc_id % dataset_size  # 或者 break
                    
            #     document_ids.append(current_doc_id)

            #     # Add the sample part
            #     offset = 0 if i > 0 else doc_index_beg_offset
            #     length = (
            #         None
            #         if i < (doc_index_end - doc_index_beg)
            #         else doc_index_end_offset + self.config.add_extra_token_to_sequence
            #     )
            #     sample_parts.append(
            #         self.dataset.get(current_doc_id, offset=offset, length=length)
            #     )
