...
def count_parameters(model, exclude_embedding=True):
    total_params = 0
    for name, param in model.named_parameters():
        # 排除词嵌入层的参数
        if exclude_embedding and ('embedding' in name.lower() or 'wte' in name or 'wpe' in name):
            continue
        # 计算可训练参数
        if param.requires_grad:
            total_params += param.numel()
    print("exclude vocab dict, the model para #:", total_params)
    return total_params
    ...
    if args.use_legacy_models:
        ...
        with build_model_context(**build_model_context_args):
            model = GPTModel(
                ...
                repeat_pattern=repeat_pattern   #revise
            )
...
if __name__ == "__main__":
    ...
    pretrain(
        ...
        forward_step,
        args_defaults={'tokenizer_type': 'GPT2BPETokenizer'},  #'RandomNumberTokenizer'},   #    ###跑random的时候改成RandomNumberTokenizer
    )
